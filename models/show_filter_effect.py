...
BIN_FILE = ROOT_DIR / "podatki" / "mic_podatki" / "pogovor_03.bin"
MIC_FVZ = 8000.0
# velikost okna 256/8000 = 32ms
FFT_W = 256
# 50% prekrivanje
FFT_OVERLAP = 0.5

# Pri 256 vzorcih in 50 % overlapu je hop size 128 vzorcev
TIME_STEP = (FFT_W * (1 - FFT_OVERLAP)) / MIC_FVZ

# DataLogger prebere binarno datoteko in jo razbije na pakete
logger = DataLogger()
raw_packets = logger.parse_file(str(BIN_FILE))
paketi = pripravi_pakete(raw_packets)

# Iz paketov sestavimo mic signal
...
# amplitudni spekter org signala 20 * log10 pretvori amplitudo v dB
power_orig = 20 * np.log10(np.abs(np.fft.rfft(original)) + 1e-10)
# amplitudni spekter filtriranega
power_filt = 20 * np.log10(np.abs(np.fft.rfft(filtered)) + 1e-10)

# delitev signala na okna 32ms
...
# za vsako okno izrac. frekvencni spekter
...
# preslikava matrike na os
extent = (0, S_orig.shape[0] * TIME_STEP, 0, fvz_mic / 2)

# figure 1 — časovna domena + spekter moči
...
# figure 2 — STFT spektrogrami
vmin = min(S_orig_db.min(), S_filt_db.min())
vmax = max(S_orig_db.max(), S_filt_db.max())

fig2, axes2 = plt.subplots(1, 2, figsize=(16, 5))
fig2.suptitle("STFT spektrogram — before/after filter", fontsize=14)
